...
class HrEmployee(models.Model):
    ...
    recurring_operation_ids = fields.One2many('hr.rule.discount', 'employee_id', string="Operaciones recurrentes")
...
class HrRuleDiscount(models.Model):
    _name = "hr.rule.discount"
    _description = "Recurring Operation"
    
    amount = fields.Float(string="Importe")
    name = fields.Many2one('hr.payslip.input.type', string="Entrada", required="1")
    code = fields.Char(string="Código", readonly=True, related='name.code')
    date_from = fields.Date(string="Fecha Inicio", required=True)
    date_to = fields.Date(string="Fecha Fin")
    employee_id = fields.Many2one('hr.employee', string="Empleado", required=True)
    apply_in = fields.Selection([('-1','Último'),('1','1er'),('2','2da'),('3','3er'),('4','4ta')], string="Aplicar en", required=True)
